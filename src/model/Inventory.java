...
public class Inventory {
    ...
    //consuming the Items
    public void itemConsumption(Product product, int taskQuantity) throws AllException.LowStockException, AllException.InvalidNumberException, AllException.InvalidQuantityException {
        ...
        synchronized (this) {
            if (checkMap(product)) {
                for (Map.Entry<Item, Integer> entry1 : itemIntegerMap.entrySet()) {
                    ...
                    if (key1.getQuantity() <= key1.getMinQuantity())
            ...
            //else: send notification to cancel or wait till there are enough
                ...
                throw new AllException.LowStockException("LOW IN STOCK,ITEMS ARE NOT ENOUGH!");
            }
        }
    }
